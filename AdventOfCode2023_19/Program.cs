...
public class PartSorter
{
    ...
    public void Sort(string input)
    {
        ...
        List<MachinePart> parts = new List<MachinePart>();
        for (; i < lines.Length; i++)
        {
            if (string.IsNullOrEmpty(lines[i]))
                break;
            var workflow = GetWorkflow(lines[i]);
        ...
        for (; i < lines.Length; i++)
        {
            var text = lines[i]
                ...
                .Replace("a", "\"a\"")
                .Replace("s", "\"s\"");
            var inputPart = JsonSerializer.Deserialize<MachinePart>(text, new JsonSerializerOptions() { });
            inputPart.Text = lines[i];
            parts.Add(inputPart);
        }
        ;
        var sum = 0;
        foreach (var part in parts)
        {
            ...
            while (currentWorkflowId != "A" && currentWorkflowId != "R")
            {
                Console.Write($"{currentWorkflowId} -> ");
                var currentWorkflow = workflows[currentWorkflowId];

                currentWorkflowId = currentWorkflow.GetDestinationFor(part);

                //currentWorkflowId = workflows[currentWorkflowId].Id;
            }
            Console.WriteLine(currentWorkflowId);
            if(currentWorkflowId == "A")
            {
                sum += part.GetSum();
            }
        }
        Console.WriteLine($"Adding all of the ratings for all of the accepted parts gives the sum total of {sum}.");
    }

    public Workflow GetWorkflow(string line)
    {
        var flow = line.Split(splitter);

        var items = flow[1].Split(',');
        var rules = items.Select(x => new WorkflowRule(x));
...
public class Workflow
{
    ...
    public IEnumerable<WorkflowRule> Rules { get; internal set; }

    public string GetDestinationFor(MachinePart part)
    {
        foreach(var rule in Rules)
        {
            if (rule.Category == null || rule.Operator == null)
            {
                return rule.Destination;
            }
            var amount = rule.GetAmount(part);
            if (rule.Operator == '>')
...
public class WorkflowRule
{
    ...
    public WorkflowRule(string line)
    {
        ...
        Category = instruction[0][0];
        Operator = instruction[0][1];
        Amount = int.Parse(instruction[0].Substring(2));
        Destination = instruction[1];
    }

    public int GetAmount(MachinePart part)
    {
        switch (Category)
        {
            case 'x':
